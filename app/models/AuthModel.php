...
class AuthModel
{
    ...
    public function register($mail, $user, $pass, $firstname, $lastname, $phone, $skill, $describe, $price = 0, $avatar = null) {
        $mail = strtolower($mail);
        $user = strtolower($user);
        $pass = password_hash($pass, PASSWORD_BCRYPT);
        $verify_key = substr(str_replace(['+', '/', '='], '', base64_encode(random_bytes(32))), 0, 8);
        db::query("INSERT INTO `users` 
        (`USERNAME`, `EMAIL`, `PASSWORD`, `FIRSTNAME`, `LASTNAME`, `PHONE`, `SKILL`, `DESCRIBE`, `PRICE`, `AVATAR`, `VERIFY_KEY`, `CREATED_AT`) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
        array($user, $mail, $pass, $firstname, $lastname, $phone, $skill, $describe, (($price == null || $price == "") ? 0 : $price), (($avatar == null || $avatar == "") ? "" : $avatar), $verify_key, date("Y-m-d H:i:s",time())));
        $mailer = new Mail();
    ...
    public function edit($id, $mail,$user,$newpass,$was_email_changed, $firstname, $lastname, $phone, $skill, $describe, $price = 0, $avatar = null) {
        if($was_email_changed) {
            $verify_key = substr(str_replace(['+', '/', '='], '', base64_encode(random_bytes(32))), 0, 8);
            if($newpass !== null) {
                $newpass = password_hash($newpass, PASSWORD_BCRYPT);
                db::query("UPDATE `users` SET `EMAIL`=?, `USERNAME`=?, `PASSWORD`=?, `VERIFY_KEY`=?, `VERIFIED`=0, `FIRSTNAME`=?, `LASTNAME`=?, `PHONE`=?, `SKILL`=?, `DESCRIBE`=?, `PRICE`=?, `AVATAR`=? WHERE ID=?", array($mail, $user, $newpass, $verify_key, $firstname, $lastname, $phone, $skill, $describe, (($price == null || $price == "") ? 0 : $price), (($avatar == null || $avatar == "") ? "" : $avatar), $id));
            } else {
                db::query("UPDATE `users` SET `EMAIL`=?, `USERNAME`=?, `VERIFY_KEY`=?, `VERIFIED`=0, `FIRSTNAME`=?, `LASTNAME`=?, `PHONE`=?, `SKILL`=?, `DESCRIBE`=?, `PRICE`=?, `AVATAR`=? WHERE ID=?", array($mail, $user, $verify_key, $firstname, $lastname, $phone, $skill, $describe, (($price == null || $price == "") ? 0 : $price), (($avatar == null || $avatar == "") ? "" : $avatar), $id));
            }
            $mailer = new Mail();
            $mailer->subject('Verify your email');
            $address = $GLOBALS["CONFIG"]["basic"]["APP_URL"]."verifymail/$verify_key";
            ...
            if($newpass !== null) {
                $newpass = password_hash($newpass, PASSWORD_BCRYPT);
                db::query("UPDATE `users` SET `EMAIL`=?, `USERNAME`=?, `PASSWORD`=?, `FIRSTNAME`=?, `LASTNAME`=?, `PHONE`=?, `SKILL`=?, `DESCRIBE`=?, `PRICE`=?, `AVATAR`=? WHERE ID=?", array($mail, $user, $newpass, $firstname, $lastname, $phone, $skill, $describe, (($price == null || $price == "") ? 0 : $price), (($avatar == null || $avatar == "") ? "" : $avatar), $id));
            } else {
                db::query("UPDATE `users` SET `EMAIL`=?, `USERNAME`=?, `FIRSTNAME`=?, `LASTNAME`=?, `PHONE`=?, `SKILL`=?, `DESCRIBE`=?, `PRICE`=?, `AVATAR`=? WHERE ID=?", array($mail, $user, $firstname, $lastname, $phone, $skill, $describe, (($price == null || $price == "") ? 0 : $price), (($avatar == null || $avatar == "") ? "" : $avatar), $id));
            }
        }
        return $newpass;
    }
