...
class Auth {
    ...
    public function register(): bool {
        if(!$this->request->workWith("POST", array("email", "username", "password", "accept", "firstname", "lastname", "phone", "skill", "describe"))) {
            //$this->captchaFailed();
        ...
        if(strlen($this->request->getPost('phone')) < 9) {
            $_SESSION["AUTH_ERROR"] = __("auth.phoneInvalid", true);
            return false;
        }
        if($this->request->getPost('price') != null && $this->request->getPost('price') < 0) {
            $_SESSION["AUTH_ERROR"] = __("auth.price", true);
            return false;
        }
        ...
        if(!empty($_FILES["avatar"]["name"])) {
            $target_dir = $_SERVER["DOCUMENT_ROOT"] . "/../public/res/img/avatars/";
            $imageFileType = strtolower(pathinfo($target_dir . basename($_FILES["avatar"]["name"]),PATHINFO_EXTENSION));
            $target_file = $target_dir . $this->request->getPost('username') . "." . $imageFileType;

            if(getimagesize($_FILES["avatar"]["tmp_name"]) === false) {
                $_SESSION["AUTH_ERROR"] = __("auth.invalidImage", true);
                return false;
            }

            if ($_FILES["avatar"]["size"] > 1000000) {
                $_SESSION["AUTH_ERROR"] = __("auth.imageSize", true);
                return false;
            }

            if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg") {
                $_SESSION["AUTH_ERROR"] = __("auth.imageType", true);
                return false;
            }


            if (move_uploaded_file($_FILES["avatar"]["tmp_name"], $target_file)) {
                $avatar = $this->request->getPost('username') . "." . $imageFileType;
            }
        }

        $p = $this->authModel->register(
            ...
            $this->request->getPost('skill'),
            $this->request->getPost('describe'),
    ...
    public function edit(): bool {
        if(!$this->request->workWith("POST", array("firstname", "lastname", "phone", "skill", "describe"))) {
            //$this->captchaFailed();
            $_SESSION["AUTH_ERROR"] = __("auth.fillAllData", true);
            return false;
        }
        ...
        $p = $this->authModel->edit(
            $this->id, 
            $mail, 
            $user, 
            $password, 
            ($this->request->getPost('email') !== $this->email),
            $this->request->getPost('firstname'),
            $this->request->getPost('lastname'),
            $this->request->getPost('phone'),
            $this->request->getPost('skill'),
